...
現在のトップページは、ブランド階層ナビではなく **「プロテインを探す」ための一覧 UI** を中心とした構成になっている。

- ヘッダー
  - タイトル: 「プロテインログ」
  - サブタイトル: 「多すぎるプロテインの最新情報 あなたにぴったりのプロテイン探し」
- PICK UP セクション
  - アマゾンランキング TOP / 急上昇プロテイン / 当サイトのレビュー最多プロテインなどを想定した **3 件の注目プロテイン**
  - 画像・メーカー名・味・カロリー・1kg あたりの価格・星評価をコンパクトに表示
- プロテイン一覧セクション（今後「プロテインを探す」に名称変更予定）
  - AI によって分類された `product_classification_results` の一覧を表示
  - 各カードには以下を表示
    - メーカー名 / 商品名 / フレーバー
      - 表示用の短いテキスト `display_manufacturer` / `display_product_name` / `display_flavor` を Gemini で生成して利用（存在しない場合は元テキストにフォールバック）
    - 1kg あたりの参考価格 (`price_per_kg`、なければ `price_jpy`)
    - 平均評価 (`avg_rating`) を 5 段階の星で表示
    - フレーバーカテゴリ（味の傾向）バッジ
      - コーヒー系 / チョコ系 / フルーツ系 / ミルク系 / お菓子系 / 食事系 / プレーン / ヨーグルト / 抹茶 など

> フィルタバー（甘さスライダー / フレーバーカテゴリのアイコン選択 / 価格帯スライダー）は UI デザインと要件が固まっており、実装は今後対応予定。
