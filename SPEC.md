...
- **自動データ更新**
  - Amazon / メーカーサイト / iHerb からのデータ収集と AI 判定は、1日1回のバッチ実行を基本とし、人手の介在を極力減らす。
  - 手動オペレーションはデバッグや一時的な再実行に限り、通常運用では Cloud Scheduler + Cloud Run Jobs による自動実行を前提とする。
- **コスト制約**
  - 初期は **月間トータル 1万円以内** のコストで運用できる構成を優先する（Supabase / GCP / SerpAPI / Apify 等）。
  - 収益（アフィリエイトなど）とユーザー数に応じて、モデルのグレードアップや処理頻度・対象サイトの拡張を行う。
- **ログイン・レコメンドの拡張余地**
  - 将来的にログイン機能を追加し、ユーザーごとの嗜好（甘さ・フレーバー・価格帯など）を保存して、レコメンドやランキングのパーソナライズを行うことを前提に、テーブル設計・API 設計を行う。
- **マネタイズ**
  - メインはプロテイン商品のアフィリエイト（メーカー / iHerb / Amazon アソシエイト等）を想定。
  - `product_url` / 将来のリンク生成ロジックは、アフィリエイトリンクに差し替え可能な抽象度で実装する。
- **コンポーネント化と部分リリース**
  - フロントエンドはセクション / カード / 一覧 / 詳細 / モーダルなどをコンポーネント化し、局所的な変更や A/B テストがしやすい構造にする。
  - バックエンドもスクレイピング / 取り込み / 分類 / マージ / 集計などをバッチ単位で分割し、一部の失敗が全体停止にならないようにする。
- **セキュリティ・法令順守**
  - Supabase の RLS、認証機構、Next.js の標準的な対策（XSS, CSRF など）を前提に設計する。
  - 日本国内の関連法令（景表法、特商法、薬機法、個人情報保護法など）に抵触する可能性がある UI 文言・データ取り扱い・トラッキング機能を追加する場合は、実装前にリスクを検討し、安全側で設計する。
- **データ量の最大化**
  - MVP では「カバレッジの広いプロテインデータベース」を最優先とし、データ収集・正規化・重複統合・栄養成分補完の改善サイクルを回す。
