...
- トップページ
  - ヒーローセクション（タイトル「プロテインログ」、サブタイトル）
  - 「PICK UP」セクション（注目のプロテイン 3 選）
  - 「プロテイン一覧」（AI で分類したプロテイン一覧、後述の検索 UI でフィルタ予定）
  - 各カードに以下を表示
    - メーカー名 / 商品名 / フレーバー（Gemini で生成した表示用テキスト `display_*` を優先）
    - 1kg あたりの参考価格 (`price_per_kg` があればそれを優先)
    - 平均評価 (`avg_rating`) を 5 段階の星で表示
    - フレーバーカテゴリ（コーヒー系 / チョコ系 / フルーツ系 / ミルク系 / お菓子系 / 食事系 / プレーン / ヨーグルト / 抹茶 など）のバッジ
