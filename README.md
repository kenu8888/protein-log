...
- **自動更新前提のデータ基盤**
  - Amazon / メーカーサイト / iHerb からのスクレイピングと AI 判定は、1日1回のバッチ（`npm run daily:full` 相当）で自動実行する構成を基本とする。
  - 人手による手入力やメンテナンスを極力減らし、必要な正規化・分類は Vertex AI / Gemini を活用して行う。
- **コスト制約**
  - 初期フェーズ（サービスリリース前後）は **月間トータル 1万円以内** を目安にインフラ / API コストを抑える。
  - 利用状況と収益（アフィリエイト等）に応じて、モデルのグレードアップや処理頻度の増加を検討する。
- **将来のユーザー機能**
  - 今後ログイン機能を追加し、ユーザーごとの好み・履歴を記録して、類似プロテインのレコメンドやパーソナライズされた一覧を実現する。
- **マネタイズ方針**
  - プロテイン・サプリのアフィリエイトリンク（メーカー / iHerb / Amazon アソシエイト等）を主要な収益源とする想定。
  - 商品ページ URL は、将来的にアフィリエイト付きリンクに差し替えられるように設計する。
- **実装方針（品質・安全性）**
  - UI / 機能はコンポーネント単位で分割し、部分的なリリースやロールバックがしやすい構造にする（全停止リスクを下げる）。
  - 考えうる脆弱性（XSS, CSRF, SQL Injection, 認証周りなど）を都度レビューし、Next.js / Supabase / GCP のベストプラクティスに沿って対策する。
  - 日本国内の法令（景表法、特商法、薬機法、個人情報保護法など）に抵触する可能性がある実装（表現、クレーム、データの扱い）がある場合は、実装前に検討・必要に応じて修正・保留する。
- **データ量の最大化**
  - MVP フェーズでは「プロテインのデータ量を増やすこと」を最優先とし、スクレイピング・AI分類・重複統合・栄養成分の補完を継続的に改善する。
