...
| ソース | 取得 | 取得元テーブル | 備考 |
|--------|------|----------------|------|
| **Amazon** | **`npm run batch:amazon-sync`**（`amazon_product_sync.py` / Playwright） | `scraped_products` | **推奨**: 検索＋詳細を Playwright で取得し価格・在庫・重量・栄養を投入。`daily:scrape` はこのスクリプト＋メーカー取得。従来の SerpAPI は `POST /api/scrape-protein`（非推奨）。拡張カラムは `AMAZON_SYNC_USE_EXTENDED_COLUMNS=1` で送信。 |
| **メーカー** | `POST /api/manufacturers/scrape` | `manufacturer_products` | MY ROUTINE は一覧＋「もっと見る」（admin-ajax.php）で全件取得し、各詳細ページから価格等をパース。**マイプロテイン**向けに AI 抽出精度テスト用 API（`POST /api/manufacturers/ai-extract-test`）があり、`/api/ai-extract-product` で栄養・価格等の取得率を検証できる。詳細は `docs/MYPROTEIN_AI_EXTRACT_TEST.md`。 |
...
| `docs/MYPROTEIN_AI_EXTRACT_TEST.md` | マイプロテイン向け AI 抽出精度テスト（実行方法・取得率確認・404 判定・debug） |
