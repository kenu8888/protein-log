...
## 直近の主な対応（一覧・メーカー取得まわり）

- **一覧**: 同一商品（表記ゆれ含む）を 1 行にまとめ、1kg あたり最安を表示。メーカー・名称ともに空の行は非表示。
- **classify API**: `productMatch`（メーカー名・商品名）または `sourceTextIds` で対象だけ再分類可能。
- **MY ROUTINE 価格・栄養**: 価格は「通常購入」ブロックを特定して取得、`11000（税込）」など全角括弧にも対応。栄養は `details` の「栄養成分表示」内 `.answer` から取得。デバッグは `GET /api/manufacturers/scrape-debug?product_url=...` でパース結果と HTML スニペットを確認可能。
- **診断**: `GET /api/manufacturers/scrape-diagnose?product=商品名` で該当の manufacturer_products と classification_results の対応を確認可能。
...
- `20260318000000_add_unit_text_to_classification_results.sql` … product_classification_results に unit_text 追加（容量別表示用）
...
| `docs/DATA_UPDATE_FULL.md` | 全データ更新（Amazon・メーカー・import・classify）の一括・手順別コマンド |
| `docs/MANUFACTURER_SCRAPE_IMPROVEMENT.md` | メーカー取得の精度改善・欠損調査・MY ROUTINE の「もっと見る」 |
| `docs/MANUFACTURER_SCRAPE_RUN.md` | メーカー一括スクレイプの実行手順・デバッグAPI・MY ROUTINE の登録 |
| `docs/MANUFACTURER_DATA_TROUBLESHOOTING.md` | メーカー取得データが画面に出ないときの切り分け・診断API（?product= で特定商品調査） |
| `docs/MY_ROUTINE_DATA_FLOW.md` | MY ROUTINE のデータ取得フロー（スクレイプ〜補完〜表示）・価格が取れないときのデバッグ |
